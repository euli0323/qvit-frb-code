...
PROJECT_ROOT = SCRIPT_DIR if (SCRIPT_DIR / "utilitys").exists() else SCRIPT_DIR.parent
sys.path.insert(0, str(PROJECT_ROOT))

from utilitys.builder import build_loss, build_model, build_optimizer, build_scheduler
from utilitys.data_process import build_datasets, build_transforms
CLASS_LABELS = {0: "frb", 1: "fri"}
DEFAULT_TEACHER_CHECKPOINT = "teacher_frb0_yxdata/resnet18_baseline/001_seed142/best_model.pth"


DEFAULT_CONFIG = {
    "experiment_name": "vit3_cloud",
    "data_dir": "data/mix_1124_0529_1913",
    "output_dir": "res_c_linear_qkv",
    "seed_base": 42,
    "seeds": 15,
    "save_model": True,
    "save_history": True,
    ...
    "dim_head_after_qkv_proj": 2,
    "qkv_mlp_hidden_dim": 2,
    "pro_attn_drop": 0.2,
    "pro_linear_drop": 0.2,
    "weight_decay": 1e-2,
    "batch_size": 32,
...
def normalize_config(config):
    ...
    config["qkv_projection"] = str(config.get("qkv_projection", "linear")).lower()
    if "dim_head_before_qkv_proj" not in config and "dim_before_qkv_proj" in config:
        config["dim_head_before_qkv_proj"] = config["dim_before_qkv_proj"]
    if "dim_head_after_qkv_proj" not in config and "dim_after_qkv_proj" in config:
        config["dim_head_after_qkv_proj"] = config["dim_after_qkv_proj"]
    if "dim_head_before_qkv_proj" not in config and "dim_head" in config:
        config["dim_head_before_qkv_proj"] = config["dim_head"]
    if "dim_head_after_qkv_proj" not in config and "qkv_linear_dim_head" in config:
        config["dim_head_after_qkv_proj"] = config["qkv_linear_dim_head"]
    if "dim_head_before_qkv_proj" in config and config["dim_head_before_qkv_proj"] is not None:
        config["dim_emb"] = int(config["dim_head_before_qkv_proj"]) * int(config["num_heads"])
    else:
        config["dim_head_before_qkv_proj"] = int(config["dim_emb"]) // int(config["num_heads"])
    if int(config["dim_emb"]) % int(config["num_heads"]) != 0:
        raise ValueError("dim_emb must be divisible by num_heads.")
    if int(config["dim_emb"]) != int(config["dim_head_before_qkv_proj"]) * int(config["num_heads"]):
        raise ValueError("dim_emb must equal dim_head_before_qkv_proj * num_heads for VIT_mix.")
    if config["qkv_projection"] == "linear":
        config["dim_head_after_qkv_proj"] = int(config.get("dim_head_after_qkv_proj", config["dim_head_before_qkv_proj"]))
    elif config["qkv_projection"] == "mlp":
        config["qkv_mlp_hidden_dim"] = int(config["qkv_mlp_hidden_dim"])
    else:
        raise ValueError("qkv_projection must be 'linear' or 'mlp'.")
    return config
...
def build_run_name(config):
    ...
    if config["qkv_projection"] == "linear":
        ...
        return f"linear_before{dim_before}_after{dim_after}"
    qkv_hidden = int(config["qkv_mlp_hidden_dim"])
    return f"mlp_before{dim_before}_hidden{qkv_hidden}"
...
def main():
    parser = argparse.ArgumentParser(description="Cloud runner for VIT3 repeated-seed experiments.")
    ...
    if args.use_teacher is True:
        ...
        config["output_dir"] = "res_c_linear_qkv_distill" if str(config.get("qkv_projection", "linear")).lower() == "linear" else "res_c_mlp_qkv_distill"
