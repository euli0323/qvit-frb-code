...
JSON_DIR_C = BASE_DIR / "res_c" / "res_test"
JSON_DIR_Q = BASE_DIR / "res_q" / "res_test"
SAVE_DIRS = {
    "png": BASE_DIR / "fig_paper2",
    "pdf": BASE_DIR / "fig_paper2",
}

METRIC_KEYS = ["acc", "recall"]
PANEL_LABELS = ["a", "b"]
HEAD_VALUES = [3, 4, 5]
...
def parse_file_head_value(path):
    match = re.search(r"_(?:c|q)(\d+)\.json$", path.name)
    if not match:
        return None
    return int(match.group(1))


def mean_std(values, metric):
    metric_values = np.array([float(trial[metric]) for trial in values], dtype=float)
...
def _load_curve_q(qubits, metric):
    json_path = JSON_DIR_Q / f"res_test_q{qubits}.json"
    ...
    for key, values in data.items():
        ...
        mean, std = mean_std(values, metric)
        means.append(mean)
        stds.append(std)
    return _sort_by_x(x_vals, means, stds)


def _load_curve_c(head_value, metric):
    json_path = JSON_DIR_C / f"res_test_c{head_value}.json"
    with json_path.open("r", encoding="utf-8") as f:
        data = json.load(f)
...
def _ylim_for_metric_curves(q_curves, c_curves, pad=0.6):
    lows, highs = [], []
...
def _draw_compare_on_ax(ax, metric):
    q_curves = [(v, *_load_curve_q(v, metric)) for v in HEAD_VALUES]
    c_curves = [(v, *_load_curve_c(v, metric)) for v in HEAD_VALUES]
    ...
    ax.set_ylim(_ylim_for_metric_curves(q_curves, c_curves))
...
def _save_figure(fig, stem):
    for ext, save_dir in SAVE_DIRS.items():
        ...
        tmp_path = save_dir / f"{stem}.tmp.{ext}"
        try:
...
def plot_fig3():
    ...
    for ax, metric, panel in zip(axes, METRIC_KEYS, PANEL_LABELS):
        handles, labels = _draw_compare_on_ax(ax, metric)
...
def main():
    setup_nature_style()
    print("[Plotting] fig3 test-set comparison")
    plot_fig3()
